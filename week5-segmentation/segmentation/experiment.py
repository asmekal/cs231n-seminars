...
from catalyst.data import (
    ImageReader, ListDataset, MaskReader, ReaderCompose, ScalarReader, LambdaReader
)
...
class BarcodeExperiment(ConfigExperiment):
    ...
    def get_datasets(
        ...
        def encode_fn_lambda(fname, datapath):
            return (cv2.cvtColor(cv2.imread(os.path.join(datapath, fname)), cv2.COLOR_BGR2GRAY) // 255)[:, :, None]
...
class VocExperiment(BarcodeExperiment):
